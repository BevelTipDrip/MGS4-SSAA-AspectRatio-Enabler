...
namespace mgspwe::keys
{
    ...
    constexpr const char* GpuLocalMinWidth = "GPU Local Minimum Width";   // Lab: how small a texture may be and still have its CPU write access stripped
    ...
    constexpr const char* MouseFractionCarry = "Mouse Fraction Carry";   // Lab for now: keep the fraction of each frame's mouse turn instead of truncating it away (docs/pw/mouse-input.md 2e)
    constexpr const char* MouseLateSample = "Mouse Late Sample";          // Lab for now: sample the mouse right before the look-input routine uses it instead of a frame earlier (docs/pw/mouse-input.md 2f)
    constexpr const char* MousePromptPump = "Mouse Prompt Pump";          // Lab for now: the mouse thread handles a message the moment it arrives instead of after one or two Sleep(1) (needs Busy Wait Fix = Everything)
